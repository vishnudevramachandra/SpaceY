...
launch_sites = pd.unique(spacex_df['Launch Site'])
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        names='Launch Site', 
        title='Total Suceess Launches By Site')
        return fig
    else:
        # return the outcomes piechart for a selected site
        filtered_df = spacex_df[spacex_df['Launch Site'].isin([entered_site])]
        fig = px.pie(filtered_df.groupby('class').count().reset_index(), 
        values='Unnamed: 0', 
        names='class', 
        title='Total Suceess Launches For Site ' + entered_site)
        return fig
...
def get_scatter_plot(entered_site, payload_mass):
    # Select the rows that fall within the range of the selected payload mass
    filtered_df = spacex_df[np.logical_and(
        spacex_df['Payload Mass (kg)'].values >= payload_mass[0],
        spacex_df['Payload Mass (kg)'].values <= payload_mass[1])]
    if entered_site == 'ALL':
